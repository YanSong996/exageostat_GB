...
//double Z_new[1440][30295];  // 82 *365  for daily
...
double MLE_alg_mean_trend(unsigned n, const double * theta, double * grad, void * MORSE_data)
	 ...
	 */
{
	...
	MORSE_request_t  *mrequest       = (MORSE_request_t *) data->request;
	int         N       = X->m;
	MORSE_desc_t *XtX=data->XtX;           // a matrix generated in this process
					       // EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&forcing, NULL, MorseRealDouble, dts,dts,dts*dts,N,N,0,0,N,1,p_grid,q_grid);
					       // EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&Zobs, NULL, MorseRealDouble, dts,dts,dts*dts,N,N,0,0,N,1,p_grid,q_grid);
...
void mean_trend(double * theta, void * MORSE_data, int l, int u)
	//! Maximum Loglikelihood Evaluation (MLE)
	/*!  -- using exact or approximation computation, and (single, double, or mixed) precision.
	 * Returns the loglikelihhod value for the given theta.
	 * @param[in] n: unsigned variable used by NLOPT package.
...
//	double *estimated_mean_trend_lapack = (double *) malloc(N * sizeof(double));
	double *estimated_mean_trend_lapack = estimated_mean_trend->mat;
...
//return;
	double *part2_vector_lapack = part2_vector->mat;
	...
	if ( MORSE_My_Mpi_Rank() == 0 )
	{
		...
		for(int year=0; year<35;year++)
		{
			for(int i = 0; i < 365*24; i++){   //for hourly , for daily, just 365
				...
				//fprintf(stderr, "part2_vector_lapack[i]: %f\n", estimated_mean_trend_lapack[iter]);				
				iter++;
			}
...
double time2 = 0.0;	
	//exit(0);
		if(l == loc_count-1)
		{
			//		for(size_t time_slot=0; time_slot<83*365*24;time_slot++)   //for daily 83*365
#pragma omp parallel for	
			for(size_t time_slot=0; time_slot<35*365*24;time_slot++)   //for daily 83*365
			{
				...
				//	size_t lineCount =0;
				//	if(time_slot <=100)
				//	size_t lineCount = count_lines_in_file(nFileZ_data);
					STOP_TIMING(time1);			
					//fprintf(stderr, "%d %d\n", lineCount, (u)*loc_count);
					//if (true /*(time_slot <=100  && lineCount == (u)*loc_count )|| time_slot>100*/) {
					if (true /*lineCount == (u)*loc_count*/ ){				
						...
						// Calculate the maximum length of each formatted double string
						//size_t max_len = snprintf(NULL, 0, "%0.14f\n", Z_new[0][time_slot]);

						// Buffer to hold double values and newline characters
						//char buffer[loc_count * (max_len + 1)];
  ...
  fprintf(fp_data, "%0.14f\n", value);
							// Format the double value as a string followed by a newline character
						//	size_t len = snprintf(&buffer[offset], max_len + 1, "%0.14f\n", value);
						//	offset += len;
						}

						// Write the buffer to the file
					//	fwrite(buffer, sizeof(char), offset, fp_data);						
						STOP_TIMING(time2);					
						//			free(buffer);
						//free(estimated_mean_trend_lapack);
						fclose(fp_data);
						//fprintf(stderr, "done\n");
						//free(nFileZ_data);
						...
						/* if (lineCount < (u)*loc_count)*/ {
							...
							//fprintf("count: %d at %d\n", lineCount, time_slot);							
							sleep(30); // Sleep for 10 seconds
						}
				}

				//				fprintf(stderr, "%d 2- %d, hi\n", l, loc_count);
				//exit(0);
				}
			...
			if(l==loc_count-1)
			{
				while(true)
				{
					...
					if (lineCount == (u)*loc_count) {
						...
						//fprintf(stderr, "linecount: %d, (u)*loc_count: %d\n", lineCount, (u)*loc_count);
						fprintf(stderr, "File does not have the expected number of lines. Waiting...\n");
						sleep(30); // Sleep for 10 seconds
					}
				}
		 ...
		 * */
	{
		...
		if(data->mean_trend ==1)
		{
			int M = data->M;
			//Fill data struct
			//	fprintf(stderr, "0000 %d, %d\n", N, (3+2*M));
			EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&X, NULL, MorseRealDouble, dts,dts,dts*dts,N,3+2*M,0,0,N,3+2*M,p_grid,q_grid);
			EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&part2_vector, NULL, MorseRealDouble, dts,dts,dts*dts,3+2*M,1,0,0,3+2*M,1,p_grid,q_grid);
			EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&XtX, NULL, MorseRealDouble, dts,dts,dts*dts,3+2*M,3+2*M,0,0,3+2*M,3+2*M,p_grid,q_grid);
