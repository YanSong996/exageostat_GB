...
int main(int argc, char **argv) {
	...
	int q_grid=1;
        ...
        MORSE_desc_t *B       = NULL;	
        MORSE_desc_t *C       = NULL;	
        EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&A, NULL, MorseRealDouble, dts, dts, dts * dts, N, N, 0, 0, N, N, p_grid, q_grid);
	EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&B, NULL, MorseRealDouble, dts, dts, dts * dts, N, 1, 0, 0, N, 1, p_grid, q_grid);
        EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&C, NULL, MorseRealDouble, dts, dts, dts * dts, N, 1, 0, 0, N, 1, p_grid, q_grid);

	MORSE_dgemm_Tile(MorseNoTrans, MorseNoTrans, 1, A, B , 0, C);// C=AB+\alpha*C
